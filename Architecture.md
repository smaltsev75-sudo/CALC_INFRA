...
**Версия 2.10.3** (Stage 17.3 — Dashboard CTA dedup). Schema v16.
...
│               markdown, providerPriceFetch. Возвращают           │
...
│               calcVersioning.js, validation.js, formula/*.       │
├──────────────────────────────────────────────────────────────────┤
...
│  (renderProviderUpdateRow — providerPriceSummary.js)                    │
...
```
domain/costOptimizationPlanner.js   — pure draft API + lever specs + recompute
controllers/costOptimizationPlannerController.js  — единственная mutation-boundary
ui/modals/costOptimizationPlannerModal.js          — draft editor (level / constraints / levers / preview)
ui/costOptimizationPlanner.js                      — teaser-card на Dashboard
```
...
[migrations.js](js/state/migrations.js) — массив `MIGRATIONS` шагов `from → to`. Текущая версия — **16** (`CURRENT_SCHEMA_VERSION` re-export от `LATEST_SCHEMA_VERSION`).
...
Модалок 26 (на 2.10.2, после Stage 17.2): formula, help, confirm, message, input, itemEdit, questionEdit, reset, assumptions, assumptionsRegister, duplicateImport, printAnswersOptions, quickStart, reapplyConfirm, scenarioMenu, scenarioRename, scenarioDuplicate, scenarioComparison, deltaHistory, providerAnalytics, calculationHealth, sensitivity, budgetGuardrails, decisionMemo, guidedCompletion, priceImportMapping.
...
```bash
npm test                  # все тесты, sequential, spec-репортер
...
Sanity-check скрипт ([_sanity-check.mjs](tests/_sanity-check.mjs), префикс `_` исключает из runner'а): прогоняет калькулятор на 3 профилях (Startup / SMB / Enterprise), пишет [SANITY_REPORT.md](SANITY_REPORT.md). Полезно после правок прайсов или формул.
...
│   │   │   details.js, comparison.js, itemsTab.js, questionsTab.js
│   │   ├── providerPriceSummary.js   # provider-row + summary
...
│   │   ├── validation.js             # типы, лимиты, висящие Q.* в формулах
│   │   ├── costType.js               # CAPEX/OPEX классификатор
│   │   ├── withoutRisks.js, scenarios.js, wizardProfiles.js
...
4. **Decision Memo** ([decisionMemoExport.js#buildSummarySection](js/services/decisionMemoExport.js)): строка `Ставка НДС: 22% (авто, дата ставки: 2026-05-12)` / `(заморожено, дата фиксации: 2024-06-01)` / `(вручную)` или `НДС: не учитывается` при `vatEnabled=false`.
