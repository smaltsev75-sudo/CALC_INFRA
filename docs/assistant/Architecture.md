...
**Версия 2.20.60** (Health gate при JSON/Quick Start, все активные вопросы Опросника либо связаны с ЭК/производным расчётом, либо явно помечены как бюджетный/плановый контроль). Schema v20.
...
| [seed-formulas.test.js](tests/unit/domain/) | Каждая qty-формула 63 ЭК парсится и вычисляется в финитное ≥0 число |
| [questionnaire-field-impact-coverage.test.js](tests/unit/architecture/) | Каждый активный вопрос Опросника либо влияет на ЭК/производный расчёт, либо явно входит в allowlist non-EK controls |
...
│   │   ├── seed.js                   # 63 ЭК + 89 вопросов + DEFAULT_SETTINGS
...
- [SANITY_REPORT.md](SANITY_REPORT.md) — последние числа калькулятора на 3 sanity-профилях; расширенные business golden snapshots покрывают 6 профилей в тестах.
...
```json
{
    ...
    "source": "Cloud.ru official public tariff documents: Evolution/Advanced",
    "vatPolicy": {
...
User-facing labels не хардкодятся рядом с таблицами: [providerPriceTrust.js](js/domain/providerPriceTrust.js)
мапит эти значения и missing/by-request gaps в русские статусы для сводки цен и
модалки сравнения провайдеров.
