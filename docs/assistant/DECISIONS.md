...
## 16.06.2026 · PATCH 2.20.119 — После импорта JSON-расчёта — переход на Дашборд

**Контекст.** После загрузки расчёта из JSON-файла («Импорт JSON») пользователь
попадал на вкладку «Опросник». По требованию пользователя — показывать сразу
результат, то есть Дашборд.

**Решение.**

- [js/app/importExportActions.js](js/app/importExportActions.js) `handleImportResult`:
  при `res.ok` `store.setActiveTab('questionnaire')` → `'dashboard'`. Активный
  расчёт уже установлен импортом, поэтому Дашборд сразу с данными (Health-gate /
  repair-диалоги, если есть, открываются поверх).
- **«Полный импорт» (bundle, замена всего состояния) не меняли** — он ведёт на
  список «Расчёты» ([:199](js/app/importExportActions.js#L199)), т.к. заменяет всё
  состояние и единого активного расчёта для Дашборда нет.
- Тесты [app-import-export-actions.test.js](tests/unit/architecture/app-import-export-actions.test.js):
  два assert на пост-импортную вкладку обновлены `['questionnaire']` → `['dashboard']`.
  E2E `desktop-data-management.spec.js` проверяет длину списка, не вкладку — без регрессии.

**Проверки.**

- `npm test`: 5642/5642 pass (EXIT=0).
- `npm run syntax-check`: pass.

**Версионирование.** `2.20.118 → 2.20.119` (PATCH). Только навигация после импорта;
schema/bundle/provider не меняются. `package-lock.json` синхронизирован.

## 16.06.2026 · PATCH 2.20.118 — Иконка приложения адаптирована под светлую тему

**Контекст.** Иконка приложения (вариант K) спроектирована с тёмной плиткой и на
светлой теме (тёплая cream-палитра) выглядела как тяжёлый тёмный блок: внутренние
детали чипа (тёмно-slate чип на navy-плитке) почти не читались, иконка смотрелась
инородно. Жалоба пользователя: «тёмную иконку плохо видно на светлой теме».

**Решение.**

- [js/ui/appIcon.js](js/ui/appIcon.js) переведён на палитры: `appIconSvg(size, variant)`
  и `appIconEl({ variant })` принимают `'dark'`/`'light'`. Light-палитра — светлая
  slate-плитка (`#f4f7fc → #e3eaf4`) с тёмной внутренней рамкой, тёмный чёткий чип,
  та же яркая lime-искра и зелёный ₽-бейдж в emerald (`#10b981 → #059669`,
  совпадает с light-accent проекта) с белым знаком ₽.
- Бренд sidebar ([sidebar.js](js/ui/sidebar.js) `renderBrand(state)`) и шапка
  Справки ([helpModal.js](js/ui/modals/helpModal.js) `renderHelpHeader(state)`)
  выбирают вариант по `state.ui.theme` — при смене темы иконка перерисовывается
  вместе со всем UI.
- **Favicon оставлен тёмным** (вкладка браузера вне темы приложения); канонический
  [assets/app-icon.svg](assets/app-icon.svg) (тёмный) не меняется.

**Проверки.**

- `npm test`: 5642/5642 pass (EXIT=0).
- `npm run syntax-check`: pass.
- Browser-smoke (свежий origin для обхода ESM-кэша): тёмная тема — тёмная плитка
  (v2.20.118); светлая тема — светлая плитка, тёмный чип, искра и ₽ читаются в
  бренде sidebar и в шапке Справки; смена темы переключает вариант.

**Версионирование.** `2.20.117 → 2.20.118` (PATCH). Только presentation иконки в
UI; schema/bundle/provider не меняются. `package-lock.json` синхронизирован.

## 16.06.2026 · PATCH 2.20.117 — Новая иконка приложения (AI-чип) + версия под иконкой

**Контекст.** По выбору пользователя из 11 предложенных концептов зафиксирован
вариант K — «AI-чип со знаком ₽» (процессор с AI-искрой в ядре + бейдж рубля),
отражающий, что калькулятор считает в т.ч. инфраструктуру под LLM/RAG. Знак ₽
на бейдже дорабатывался дважды: перекладина «Р» переехала с уровня петли на
голую ножку ниже с чистым зазором (раньше при штрихе 13px нижняя кромка петли и
перекладина сливались в кляксу).

**Решение.**

- **Единый источник** иконки: канонический [assets/app-icon.svg](assets/app-icon.svg)
  + встраиваемый модуль [js/ui/appIcon.js](js/ui/appIcon.js) (`appIconSvg`/`appIconEl`).
  id градиентов/фильтра параметризованы счётчиком — иконка может быть в DOM в
  нескольких экземплярах одновременно (бренд sidebar + открытая Справка), а
  одинаковые id ломали бы `url(#...)`.
- **Favicon** ([index.html](index.html)) — статический `data:image/svg+xml`,
  сгенерирован из asset минификацией + URL-кодированием (`< > # % пробел`).
  Сохранена прежняя архитектура (data-URI под CSP `img-src 'self' data:`,
  работает и на `file://`).
- **Бренд в sidebar** ([sidebar.js](js/ui/sidebar.js) `renderBrand` + [sidebar.css](css/sidebar.css)):
  иконка `zap` заменена на иконку приложения; брендовая зона переведена в колонку
  «иконка → версия». Номер версии (`v2.20.117`) стал **видимой** подписью под
  иконкой (узкий 64px rail → компактный mono с `nowrap`). Текст ровно `v<версия>`
  сохранён — на него завязан [published-smoke.spec.js](tests/e2e/published-smoke.spec.js).
- **Первая страница Справки** ([helpModal.js](js/ui/modals/helpModal.js) `renderHelpHeader`
  + [modals.css](css/modals.css) `.help-title-card`): брендовая шапка «иконка +
  Калькулятор инфраструктуры + Версия 2.20.117» над содержимым UserManual.
- **Чистка папки**: удалена untracked-папка отбора `docs/mockups/app-icon/`
  (11 концептов + 3 preview) — выбор завершён, финал живёт в asset + модуле.
- **Тест** [a11y-batch7.test.js](tests/unit/architecture/a11y-batch7.test.js):
  регэксп импорта HOTKEYS ослаблен для допуска со-импортов (`APP_NAME`/`APP_VERSION`).

**Проверки.**

- `npm test`: 5642/5642 pass (EXIT=0).
- `npm run pages:build`: pass (EXIT=0).
- `npm run sanity:check`: pass (EXIT=0).
- `npm run syntax-check`: pass (EXIT=0).
- Browser-smoke (локальный HTTP-сервер): favicon-тег корректен; бренд показывает
  иконку + `v2.20.116`→`v2.20.117`; Справка показывает брендовую шапку; консоль
  приложения без ошибок (обе иконки в DOM одновременно — id-коллизий нет).

**Версионирование.** `2.20.116 → 2.20.117` (PATCH). Schema v20, bundle format и
provider JSON schema не меняются. Видимое изменение — иконка приложения (вкладка
браузера, sidebar, Справка) и видимый номер версии под иконкой. `package-lock.json`
синхронизирован `2.20.114 → 2.20.117` (отставал).

## 14.06.2026 · PATCH 2.20.116 — Паспорт ПРОМ: светлая тема без просвечивания backdrop

**Контекст.** В светлой теме Паспорта ПРОМ правая панель и часть контейнеров
визуально проваливались в тёмный backdrop: текст и карточки выглядели мутными,
а детализация расчёта теряла читаемость.

**Корень.** В CSS Паспорта использовался `var(--bg)`, но такого theme-токена в
проекте нет. Из-за этого `background` у `.pp-modal`, `.pp-left` и `.pp-right`
становился невалидным, а поверхности фактически просвечивали overlay.

**Решение.**

- `.pp-modal` и `.pp-left` переведены на `var(--bg-main)`.
- `.pp-right` и `.pp-cost` получили явный `var(--bg-card)` без прозрачных
  градиентов.
- Добавлены regression guards:
  - unit source-test на запрет `var(--bg)` в CSS Паспорта;
  - e2e в светлой теме, проверяющий непрозрачный фон `.pp-modal`, `.pp-left`,
    `.pp-right`.

**Проверки.**

- Targeted UI unit: 19/19 pass.
- Targeted Playwright Passport PROM: 5/5 pass.
- `npm run syntax-check`: pass.
- `git diff --check`: pass.

**Версионирование.** `2.20.115 → 2.20.116` (PATCH). Schema, bundle format и
provider JSON schema не меняются.

## 14.06.2026 · PATCH 2.20.115 — аудит расчётов, числового ввода, UI-фокуса и Pages-gate

**Контекст.** Жёсткий аудит приложения после серии правок по Паспорту ПРОМ,
Дашборду, Детализации и релизной дисциплине. Цель — не косметика, а закрытие
реальных рисков: неверное масштабирование отдельных AI-ресурсов, ошибочный разбор
чисел с пробелами, фокус поиска в модалках, контраст и Pages-деплой без CI-gate.

**Решение.**

1. **AI-ресурсы с обычными ресурсными метриками.** ЭК категории AI, которые
   являются обычной инфраструктурой (`dashboardResource`, без `dashboardAiMetric`),
   теперь масштабируются по коэффициентам ресурсов стендов, а не по AI-факторам
   нагрузки. Регрессия закрыта тестами на GPU/resourceRatio и сохранение
   AI-факторов для явных AI-метрик.
2. **Локализованный разбор чисел.** Вынесен `parseLocalizedNumber`: строки вида
   `1 000`, `1 000`, `1 000` читаются как 1000, а не как 1; неполные numeric-prefix
   строки больше не проходят как валидное число. Используется в DSL evaluator и
   repair-пайплайне загрузки JSON.
3. **Фокус поиска в модалках.** `Ctrl+Alt+F` теперь предпочитает видимый поиск
   внутри активной модалки, включая Паспорт ПРОМ, и не уводит пользователя в
   фоновый поиск Детализации.
4. **UI/a11y cleanup.** Исправлены контрастные места Паспорта ПРОМ и переключателей
   стендов, устранены tooltip-дубли в подтабах Детализации.
5. **Pages release gate.** GitHub Pages публикуется после успешного workflow `CI`,
   checkout идёт по проверенному commit SHA, после deploy запускается
   `smoke:published` по фактическому URL.
6. **Документация.** UserManual и assistant-документы синхронизированы с текущими
   UI-путями: лицензия, price benchmark, PDF/печать, published-smoke.

**Проверки.**

- `node tests/run.js`: 5636/5636 pass.
- `npm run syntax-check`: pass.
- `npm run smoke:desktop`: 59/59 pass.
...
**Версионирование.** `2.20.114 → 2.20.115` (PATCH). Schema остаётся v20; bundle
format и provider JSON schema не меняются.

## 31.05.2026 · PATCH 2.20.103 — Прозрачность коэффициентов: stale-комментарий + «Допущения по умолчанию» в Decision Memo

**Контекст.** Аудит обоснованности расчётных коэффициентов (по запросу — «особенно
подходы, которые предложил ИИ, а не человек»). Вердикт: часть коэффициентов —
инженерные оценки ИИ без citable-источника (риск-буферы 30/15/10/5/15%,
`aiStandFactor.DEV=0.02` «индустриальный ориентир», стенды ИФТ/ПСИ), но все они
второстепенны по влиянию (бюджет доминируют пользовательские входные данные +
реальные прайсы), редактируемы, а математика (мультипликативный стек рисков
→ ×1.726, инфляция `^годы`, амортизация oneTime, НДС ×1) корректна. Две правки:

1. **Stale-комментарий ([constants.js:607](js/utils/constants.js#L607)).**
   `DEFAULT_K_SCHEDULE_SHIFT` гласил «Применяется к LOAD-стенду и ко всем oneTime»
   — но код применяет сдвиг ТОЛЬКО к `oneTime` ([calculator.js:169](js/domain/calculator.js#L169),
   привязку к LOAD убрали в 13.U10). Комментарий приведён в соответствие с кодом.
2. **«Допущения по умолчанию (инженерная оценка, не норматив)» в Decision Memo.**
   В §3 «Основные параметры» memo ([decisionMemoExport.js](js/services/decisionMemoExport.js)
   `buildDefaultCoefficientsLines`) добавлен видимый список: риск-коэффициенты
   (буфер задач/проекта, инфляция, сезонность, сдвиг расписания «только разовые
   затраты», непредвиденные) — когда режим «с рисками»; AI-факторы стендов — когда
   AI используется. Каждое значение помечается `(по умолчанию)` (ещё не уточнено)
   vs `(уточнено)`; внизу дисклеймер «типовые IT-оценки, заложенные в модель
   разработчиком, а не отраслевой норматив — проверьте под конкретный проект».
   Значения читаются из `calc.settings` (фактические, не хардкод). Помогает
   согласующему бюджет видеть, какие числа — инженерная оценка ИИ.

**Метрики.** Unit 5367/5367 PASS, syntax clean, desktop e2e 55/55. 0 регрессий.
Новый тест [decision-memo-default-coefficients.test.js](tests/unit/services/decision-memo-default-coefficients.test.js).
PATCH bump 2.20.102 → 2.20.103.
...
вплотную к градиентной полосе структуры расходов (общий `gap` родителя —
только 3px). Пользователь попросил опустить эти плашки немного ниже бара.

**Решение.** К контейнеру `.dash-hero-cost-type-rows` добавлен `margin-top: 6px`.
Промежуток «бар → плашки CAPEX/OPEX» вырос с 3px до 9px, при этом отступ
«подпись «Структура расходов» → бар» остался 3px, а строка `НДС/Риски` сохранила
свой отступ ниже. Изменение чисто стилевое, структура DOM не менялась.

**Защита.** Проверено в браузере на реальном расчёте: измерены промежутки
(label→bar = 3px без изменений, bar→rows = 9px) и снят скриншот Hero. E2E-контракт
Dashboard (общий wrapper пар) не затронут, 5339/5339 unit-тестов зелёные.

## 27.05.2026 · PATCH 2.20.99 — Hero paired finance rows

**Контекст.** В карточке `Итого по расчёту` строки `CAPEX`, `OPEX`, `НДС` и
`Риски` занимали четыре отдельные горизонтальные строки. Пользователь попросил
сжать их: `CAPEX/OPEX` в одну строку, затем `НДС/Риски` тоже в одну строку.

**Решение.**

- Добавлен `.dash-hero-cost-type-rows`: `CAPEX` и `OPEX` лежат двумя колонками
  под общей полосой структуры расходов.
- Добавлен `.dash-hero-breakdown-rows`: `НДС` и `Риски` лежат двумя колонками
  в следующей строке, сохраняя inactive-state и зачёркивание только numeric
  полей.
- На узких hero-карточках обе группы возвращаются в одну колонку через
  container query, чтобы не получить горизонтальный overflow.

**Защита.** E2E-контракт Dashboard проверяет, что пары живут в общих wrappers,
находятся на одной визуальной линии, не переполняют карточку, а суммы и
проценты остаются на одной строке и выровнены внутри своих left/right колонок.

## 27.05.2026 · PATCH 2.20.98 — Hero alternate totals readability

**Контекст.** После переноса дневной/месячной/годовой суммы в одну верхнюю
KPI-строку альтернативные суммы справа стали слишком мелкими и плохо
читались.

**Решение.**

- Увеличен шрифт `.dash-hero-alt-value` с `0.72rem` до `0.86rem`.
- Увеличена подпись периода `.dash-hero-alt-label` с `0.66rem` до `0.74rem`.
- Сохранены компактные обозначения `/д`, `/мес`, `/год`, чтобы более крупный
  текст оставался в одной KPI-строке и не сбивал верхние composition bars.

**Защита.** Повторно проверяются `dashboard-risk-bars-layout.spec.js`,
desktop viewport smoke и полный `smoke:desktop`, потому что изменение размера
влияет на hero/bar geometry.

## 27.05.2026 · PATCH 2.20.97 — Hero top KPI totals

**Контекст.** В `Итого по расчёту` альтернативные периоды (`в день`, `в год`
или `в месяц`) занимали две отдельные строки ниже НДС/Рисков. Пользователь
выбрал вариант A: разместить суммы в одной строке наверху рядом с основной
суммой.

**Решение.**

- Основная сумма выбранного периода остаётся крупной слева.
- Два альтернативных периода перенесены в `.dash-hero-main` справа, меньшим
  моноширинным текстом, без отдельных карточечных строк.
- Нижний `.dash-hero-alt` блок под breakdown удалён, чтобы не дублировать
  одну и ту же информацию в двух местах.

**Защита.** E2E-контракт Dashboard проверяет, что две `.dash-hero-alt-value`
находятся внутри `.dash-hero-main` и лежат на той же визуальной KPI-строке.

## 27.05.2026 · PATCH 2.20.96 — Hero VAT inactive state

**Контекст.** В карточке `Итого по расчёту` строка `Риски` уже показывала
потенциальную сумму и зачёркивала её, если риск-коэффициенты выключены. НДС
вёл себя иначе: при расчёте без НДС справочная строка НДС не получала такого
визуального состояния.

**Решение.**

- Для `vatEnabled=false` hero-card считает потенциальный НДС от текущего итога
  без НДС (`total * vatRate`) и оставляет строку `НДС` видимой.
- В inactive VAT-состоянии зачёркиваются только сумма и процент; label `НДС`
  не зачёркивается, по тому же правилу, что label `Риски`.
- При обратном включении НДС потенциальный класс снимается, а сумма снова
  отображается как активная часть итога.

**Защита.** E2E-контракт `dashboard-risk-bars-layout.spec.js` расширен:
проверяет `БЕЗ НДС`, видимость `dash-hero-breakdown-row-vat-potential`,
зачёркивание только numeric-полей и отсутствие зачёркивания после включения
НДС обратно.
...
несколько визуальных языков: diagnostics-таблицу, отдельный primary CTA-блок
`Следующий шаг`, вторичный teaser `Оптимизация стоимости` и правую подпись
`Дата актуальности`, которая дублировала дату в тексте строки прайса.

**Решение.**

- `Дата актуальности` удалена из правой колонки строки `Прайс расчёта`; дата
  остаётся один раз в тексте `Актуальность прайса: <date>`.
- `Следующий шаг` и `Оптимизация стоимости` объединены в общий список action-
  строк с той же грамматикой, что diagnostics: область слева, описание в центре,
  компактное действие справа.
- Огромный зелёный primary CTA внутри карточки заменён на компактное действие
  того же размера, что соседние кнопки, чтобы карточка не разваливалась на
  разные стили.

**Защита.** Добавлены проверки в `dashboard-state-summary-layout.spec.js` и
`provider-price-trust-ui.test.js`: внутри карточки не должно быть видимого
дубля `Дата актуальности`, desktop/mobile layout остаётся без horizontal
overflow. Визуально проверены screenshots dark/light.
...
разрозненных вложенных карточек: двухколоночные diagnostics, тяжёлый
`Следующий шаг`, отдельный dashed-блок оптимизации и длинная строка уровней
`Консервативный · Амбициозный · Экстремальный`. На dashboard это спорило с
новой грамматикой карточек `Итого`, `Распределение` и `Вклад риск-коэффициентов`.

**Решение.**

- Diagnostics переведены в единый вертикальный список сигналов с общими
  колонками `область → состояние → действие`.
- `Следующий шаг` и `Оптимизация стоимости` оформлены как action-строки одного
  паттерна, а не как две независимые карточки внутри карточки.
- Убрана видимая строка уровней оптимизации из компактного teaser; уровни
  остаются в модалке планера, где пользователь реально выбирает сценарий.
- Для desktop ширины кнопки остаются в строке справа; мобильный fallback
  включается только на узкой карточке.

**Защита.** Добавлен E2E `dashboard-state-summary-layout.spec.js`: проверяет
вертикальный список diagnostics, порядок action-блоков и отсутствие
горизонтального overflow на desktop и mobile. Визуально проверены dark/light и
mobile screenshots.
...
`Объёмы ресурсов · ИТОГО` и `Объёмы AI-нагрузки · ИТОГО` находились рядом с
hero, но визуально всё ещё читались как отдельные карточки экрана. Пользователь
уточнил, что эти блоки должны быть именно внутри рамки `Итого по расчёту`.
Также слово `Риски` не должно перечёркиваться: inactive-состояние относится
к числам потенциальной наценки, а не к названию строки. Дополнительно суммы
`в день` / `в месяц` уходили на правый край и ломали вертикальную колонку денег.

**Решение.**

- Удалён промежуточный `dash-summary-stack`; `.dash-dashboard-metrics` теперь
  рендерится прямым дочерним блоком `.dash-card-hero`.
- `Объёмы ресурсов · ИТОГО` и `Объёмы AI-нагрузки · ИТОГО` физически находятся
  внутри border карточки `Итого по расчёту`.
- В режиме `БЕЗ РИСКОВ` перечёркиваются только сумма и процент строки `Риски`;
  label `Риски` остаётся обычным текстом.
- Строки альтернативных периодов получили ту же сетку `amount + percent-slot`,
  что `НДС`, `Риски` и CAPEX/OPEX, поэтому правый край всех сумм совпадает.
- Layout-test по risk segments проверяет видимые положительные сегменты и не
  падает на transient zero-geometry в Chromium.

**Защита.** Focused Playwright `dashboard-risk-bars-layout.spec.js` проверяет:
top-row alignment, физическое нахождение total metrics внутри hero, отсутствие
line-through на label `Риски`, line-through на inactive numeric values и общий
правый край сумм в hero. Визуально проверены dark/light screenshots.
...
`Объёмы ресурсов · ИТОГО` и `Объёмы AI-нагрузки · ИТОГО` оказались отдельной
широкой строкой сетки. Визуально они перестали относиться к карточке
`Итого по расчёту`, хотя по смыслу являются её total-scope продолжением.
Дополнительно в режиме `БЕЗ РИСКОВ` строка `Риски` показывала потенциальную
сумму как обычную активную строку.

**Решение.**

- Добавлен левый `dash-summary-stack`: hero `Итого по расчёту`, суммарные
  ресурсы и суммарная AI-нагрузка теперь находятся в одном DOM/layout scope.
- Total resource/AI блоки больше не занимают page-wide row и не растягивают
  соседние карточки `Распределение` / `Вклад риск-коэффициентов`.
- Для top composition cards сохранена единая высота на wide desktop, но
  соседние карточки не тянутся на высоту всего левого stack.
- В hero строка `Риски` получает line-through только когда
  `applyRiskFactors=false`; при включённых рисках перечёркивание отсутствует.
- Hardware total grid уплотнён так, чтобы шесть коротких ресурсных tiles чаще
  помещались в одну строку внутри левого summary-stack.

**Защита.** E2E `dashboard-risk-bars-layout.spec.js` проверяет, что
`.dash-dashboard-metrics` находится внутри `dashboard-summary-stack`, не
уезжает под карточку категорий, верхние карточки сохраняют одинаковую высоту,
а строка `Риски` перечёркнута только в режиме `БЕЗ РИСКОВ`. Визуально
проверены dark/light screenshots и отдельный screenshot режима без рисков.
...
указал на два визуальных дефекта: в `Вклад риск-коэффициентов` процент мог
переноситься под сумму, хотя в `Распределение по категориям` он стоит в той же
строке; карточка `Итого по расчёту` не должна раздуваться пустотой и смешивать
бюджетный hero с ресурсными блоками.

**Решение.**

- В строках `Распределение по категориям` и `Вклад риск-коэффициентов` сумма и
  процент закреплены как одна числовая пара. Даже в узком container-breakpoint
  переносится label, а не процент под сумму.
- `Итого по расчёту` переведена на компактные строки `НДС`, `Риски`,
  соседние периоды и `CAPEX/OPEX`; tooltip'ы новых строк не повторяют уже
  видимую сумму.
- Блоки `Объёмы ресурсов · ИТОГО` и `Объёмы AI-нагрузки · ИТОГО` оставлены в
  отдельной строке Dashboard без взаимного растягивания по высоте.
- Составные сегменты категорий и рисков сделаны non-shrinking, а risk-card
  фильтрует только реально нулевые вклады. Малые положительные доли больше не
  схлопываются в 0 px в параллельном Playwright-прогоне.

**Защита.** E2E layout-contract теперь проверяет, что в обеих карточках
`сумма + процент` остаются на одной линии, нет overflow/collisions, а верхние
три карточки совпадают по top/height на wide desktop. Проверены dark/light
screenshots, полный unit-набор и desktop smoke.

## 27.05.2026 · PATCH 2.20.84 — Dashboard top row visual alignment

**Контекст.** После редизайна `Распределение по категориям` и
`Вклад риск-коэффициентов` карточка `Итого по расчёту` оставалась в старом
hero-стиле и была заметно выше соседей из-за встроенных блоков ресурсов и
AI-нагрузки.

**Решение.**

- `Итого по расчёту` переведена на тот же композиционный язык: сумма,
  составная CAPEX/OPEX шкала и компактные строки с суммой и долей.
- Блоки `Объёмы ресурсов · ИТОГО` и `Объёмы AI-нагрузки · ИТОГО` вынесены в
  отдельную строку Dashboard ниже верхней тройки. Это сохраняет видимость
  ресурсов/токенов, но больше не раздувает бюджетную карточку.
- На desktop-раскладке верхние три карточки выравниваются по одной высоте.

**Защита.** Добавлен E2E layout-contract, который проверяет одинаковый top и
height для `Итого`, `Распределение` и `Вклад рисков` на wide desktop. Обновлены
token/resource DOM-контракты под новый total-scope контейнер
`.dash-dashboard-metrics`. Визуально проверены dark/light screenshots.

## 27.05.2026 · PATCH 2.20.83 — Category distribution composition UI

**Контекст.** Пользователь попросил привести карточку
`Распределение по категориям` к стилистике уже выбранной карточки
`Вклад риск-коэффициентов`: убрать повторяющиеся мини-бары в строках и показать
распределение спокойнее, как единую композицию.

**Решение.** Карточка сохраняет заголовок, бейдж `ИТОГО`, подпись scope
`Сумма по N активным стендам` и tooltips категорий. В тело добавлена крупная
итоговая сумма за выбранный период, одна составная шкала долей категорий и
строки-карточки с цветной точкой, названием, суммой и процентом.

**Защита.** Обновлены тёмная/светлая тема и print-стили под
`.dash-category-segments`. E2E layout-contract теперь проверяет и risk-card, и
category-card: ширину составной шкалы, отсутствие overflow и коллизий между
названием, суммой и процентом. Визуально проверены dark/light screenshots.

## 27.05.2026 · PATCH 2.20.82 — Risk card duplicate copy removal

**Контекст.** Пользователь указал на нарушение UX-правила: карточка
`Вклад риск-коэффициентов` в режиме выключенных рисков дважды писала одну
мысль — `Потенциальная наценка, если применить риски` в подзаголовке и
отдельную строку `если применить` под крупной суммой.

**Решение.** Нижняя строка `если применить` удалена из DOM вместе с CSS
`.dash-risk-surplus-note`. Контекст режима остался один раз — в подзаголовке
карточки.

**Защита.** Прицельные UI/unit проверки, e2e layout-contract и визуальная
проверка карточки в тёмной и светлой теме.

## 27.05.2026 · PATCH 2.20.81 — Risk contribution card composition UI

**Контекст.** Пользователь выбрал новый вариант карточки
`Вклад риск-коэффициентов`: вместо четырёх повторяющихся progress-bar строк
нужна более спокойная аналитическая компоновка. Важное уточнение пользователя:
бейдж `ИТОГО` нужно оставить.

**Решение.**

- Карточка дашборда сохраняет заголовок `Вклад риск-коэффициентов`, бейдж
  `ИТОГО` и info-tooltip.
- Потенциальная/фактическая наценка вынесена крупной суммой в тело карточки,
  рядом остаётся процентная pill-метка.
- Четыре отдельные полосы заменены одной составной шкалой долей факторов.
  Строки ниже показывают фактор, сумму и долю в общей наценке; подробная
  формула остаётся в tooltip строки.
- Карточка больше не растягивается по высоте соседней колонки дашборда:
  пустое пространство ушло из самой карточки.
- Обновлены светлая тема и print-стили под новый track `dash-risk-segments`.

**Защита.** Обновлён e2e-контракт `dashboard-risk-bars-layout.spec.js`: теперь
он проверяет составную шкалу, отсутствие overflow и коллизий между label,
суммой и процентом. Прицельные UI/unit проверки и syntax-check зелёные.

## 26.05.2026 · PATCH 2.20.80 — Commercial DB license opt-in

**Контекст.** Пользователь заметил, что `Лицензии` выглядят слишком большими:
в реальном расчёте `.playwright-mcp/user-actual-v2.json` категория давала
`991 399 ₽/мес`, из них `865 895 ₽/мес` приходилось на `СУБД (на vCPU)`.
Арифметика была корректной, но бизнес-логика ошибочной: формула всегда считала
коммерческую СУБД, хотя описание ЭК говорит «применять, когда используется
коммерческая лицензируемая СУБД».

**Решение.**

- Добавлен вопрос `db_commercial_license_required` в секцию `data_storage`.
  Default/unknown = `false`: open-source PostgreSQL/MySQL или managed DB
  провайдера не получают отдельную лицензию СУБД.
- Формулы `license-db-per-vcpu` теперь обёрнуты в явный opt-in. Для legacy
  JSON/localStorage ЭК добавлен в refresh-list `prepareLoadedCalc`, а вопрос
  подмешивается в старые словари.
- Quick Start автоматически включает флаг только для `fintech` и `b2g`
  профилей, где коммерческая/сертифицированная СУБД является разумным
  conservative-default. Обычные corporate/B2B/B2C профили оставляют флаг
  выключенным.
- Для расчёта пользователя итог изменился с `2 549 336` до `1 683 441 ₽/мес`,
  а `Лицензии` — с `991 399` до `125 504 ₽/мес`. Остаток лицензий объясняется
  ОС (`115 900 ₽/мес`) и СЗИ (`9 604 ₽/мес`) на 38 узлов по всем стендам.

**Защита.** Добавлены unit/loaded-calc тесты на opt-in и legacy refresh,
обновлены Quick Start/manual golden snapshots, wizard acceptance `59 из 90`,
tooltip coverage, `SANITY_REPORT.md` и `QUANTITY_LOGIC_AUDIT.md`.

## 26.05.2026 · PATCH 2.20.79 — Audit of non-AI budget formulas

**Контекст.** После исправления AI/LLM бюджета пользователь попросил
проверить остальные статьи: `Услуги`, `Безопасность`, `Трафик`,
`Аппаратные ресурсы`, `Лицензии`, включая ошибки формул и логики расчёта.
Аудит выполнялся на реальном JSON `.playwright-mcp/user-actual-v2.json` через
полный load pipeline `prepareLoadedCalc`, Cloud.ru provider overlay и
`auditQuantityLogic`.

**Результат проверки.**

- Арифметика по пяти статьям сходится: каждая применимая ячейка проходит
  `qty × effective price × billing interval × VAT/risk`, а суммы категорий
  сходятся с итогом. Для расчёта пользователя: `HW 145 713 ₽`,
  `LICENSE 991 399 ₽`, `TRAFFIC 168 863 ₽`, `SERVICES 855 310 ₽`,
  `SECURITY 362 163 ₽`.
- Health Check на этом расчёте остаётся без ошибок: только предупреждения про
  явный egress `15 ТБ/мес` против автооценки `~3.9 ТБ/мес` и сезонность при
  отключённых риск-коэффициентах.
- Найден формульный дефект в `service-external-api-calls-1m`: стенд `LOAD`
  использовал `round(...)`, тогда как `IFT/PSI/PROD` и тарифная единица
  `1 млн вызовов` требуют округления вверх. На `1.4 млн` вызовов LOAD давал
  `1` пакет вместо `2`.

**Решение.**

- Формула `service-external-api-calls-1m.LOAD` переведена с `round(...)` на
  округление вверх с capped LOAD-множителем: коэффициент выше `1.0` не
  умножает внешние API автоматически, чтобы generic stress-LOAD не раздувал
  платные вызовы сторонних провайдеров без явного monthly-объёма.
- ЭК добавлен в refresh-list `prepareLoadedCalc`, чтобы старые сохранённые
  JSON/localStorage расчёты подхватывали исправление, а не оставались на
  embedded legacy-формуле.
- Добавлен regression-test на новый seed и отдельный loaded-calc test для
  legacy `round(...) → ceil(...)`.
- Подсказка RAM больше не содержит жёсткий пример `56 ГБ → 68 ГБ`, потому что
  он был корректен только для одного набора входных данных и выглядел как
  противоречие при других расчётах.
- Health Check больше не называет `users_total` DAU/active-base: это
  накопленная аудитория за срок жизни продукта. Проверка
  `registered_users_total > users_total` теперь ловит реальное противоречие
  входных данных.
...
в основной таблице подписи `qty` / `₽/мес` выводились прямо под названиями
стендов, хотя рядом уже есть явный столбец `Ед.изм.`, а в `Сводка AI-метрик`
единица была спрятана в первом столбце `Метрика / ед.`.

**Решение.**

- В основной Детализации stand-заголовки и qty-ячейки теперь показывают только
  значения; единицы не рендерятся в `.col-stand` и живут в `Ед.изм.`.
- `Сводка AI-метрик` разделяет первый столбец на `Метрика` и `Ед.изм.`.
- Заголовки `Ед.изм.` в основной таблице и AI-сводке получили явное
  горизонтальное выравнивание, чтобы два блока выглядели как единая таблица.

## 25.05.2026 · PATCH 2.20.68 — RAM Нагрузки, AI-сводка и risk-card layout

**Контекст.** После фикса HDD пользователь показал новый расчётный дефект:
при RAM ПРОМ `56 ГБ` и коэффициенте RAM Нагрузки `120%` Дашборд показывал
`66 ГБ`, хотя ожидаемый платный объём — `ceil(56 × 1,2) = 68 ГБ`. На том же
экране `Сводка AI-метрик` оставалась непонятной по единицам и не была жёстко
согласована с колонками основной таблицы Детализации, а компактная карточка
«Вклад риск-коэффициентов» визуально раскладывала суммы неаккуратно.

**Решение.**

- `ram-gb` теперь масштабирует полный ПРОМ-бейзлайн RAM:
  `ceil(PROМ vCPU) × RAM/vCPU + cache`, затем коэффициент ресурса стенда.
  Это убирает промежуточное округление `ceil(CPU × standRatio)`, из-за
  которого `56 × 1,2` превращалось в `66`.
- `formulaHelp` для RAM фиксирует пользовательский контрольный пример:
  `56 ГБ → 68 ГБ` при Нагрузке 120%.
- Legacy/imported JSON защищены текущим refresh-list: `ram-gb` уже входит в
  `_AGENT_FORMULA_REFRESH_IDS`, поэтому `prepareLoadedCalc` подхватывает
  новую формулу при загрузке старого расчёта.
- `Сводка AI-метрик` получила явный первый столбец `Метрика / ед.` и единицы
  в каждой строке: в бюджете `₽/мес`, в qty — нативные единицы метрики.
- Колонки стендов в AI-сводке выравниваются по фактическим координатам
  основной таблицы Детализации выше; e2e guard сверяет `left/width` по
  стендовым заголовкам и колонке `ИТОГО`.
- Денежная AI-сводка суммирует display-rounded рубли так же, как видимые
  строки ЭК в таблице Детализации. Это убирает расхождение на 1 ₽ между
  строками и сводкой.
- Risk-card в компактной ширине использует фиксированную дорожку бара и одну
  строку `bar / сумма / процент`, чтобы бары имели одинаковую длину и не
  наезжали на суммы.
- Добавлены/усилены guards: domain `ram-load-ratio`, e2e Dashboard RAM/HDD
  120%, e2e Dashboard/Details token budget reconciliation, e2e cross-table
  alignment для AI-сводки и e2e risk-bar layout.

## 25.05.2026 · PATCH 2.20.67 — HDD Нагрузки масштабирует полную ПРОМ-базу

**Контекст.** При коэффициенте Нагрузочного стенда 120% пользователь увидел,
что HDD на Нагрузке меньше HDD на ПРОМ. Арифметика коэффициента работала, но
формулы были несопоставимы: ПРОМ HDD включал бэкапы БД и холодную долю файлов,
а Нагрузка масштабировала только бэкапы БД.

**Решение.**

- `storage-hdd-tb.LOAD` теперь использует ту же cold-storage базу, что и ПРОМ:
  бэкапы БД + 50% холодной доли файлов, затем применяет коэффициент стенда.
- `formulaHelp` и UserManual уточняют, что HDD Нагрузки масштабирует
  сопоставимую ПРОМ-базу.
- Legacy/imported JSON защищён существующим refresh-list:
  `storage-hdd-tb` обновляется через `prepareLoadedCalc`.
- Добавлены regression guards: domain seed test, unit load-pipeline refresh
  test и e2e по карточкам Дашборда: при HDD=120% Нагрузка > ПРОМ и близка к
  `ПРОМ × 1.2`.
- На пользовательском JSON `repaired-v2.20.66` контрольный расчёт после refresh:
  HDD ПРОМ `3.039 ТБ`, HDD Нагрузка `3.647 ТБ`.

## 25.05.2026 · PATCH 2.20.66 — явные токены, выравнивание Детализации и risk-бары

**Контекст.** В пользовательском расчёте раздел «Объём токенов» был заполнен,
но Dashboard и Детализация показывали `—` по токенам, если сохранённый/legacy
master-флаг `ai_llm_used` оказался выключен или устарел. На том же экране
заголовки стендов в Детализации не совпадали с правым выравниванием числовых
колонок, а в карточке «Вклад риск-коэффициентов» бары могли визуально упираться
в суммы на узкой карточке.

**Решение.**

- Доменный fallback внешних LLM-token ЭК теперь учитывает явный пользовательский
  объём токенов: положительные `registered_users_total`, DAU, доля AI,
  запросы/день и input/output tokens активируют расчёт токенов даже при
  устаревшем `ai_llm_used=false`. Для `on_prem_gpu` внешний token-биллинг
  остаётся 0.
- Dashboard AI/resource aggregates используют тот же критерий явного token
  demand, поэтому «Токены» видны в «Объёмы ресурсов», «Объёмы AI-нагрузки»,
  карточках стендов и `Детализация → Сводка AI-метрик`.
- Health Check `ai-token-volume-without-llm` переведён в warning: расчёт теперь
  не обнуляет токены, но просит уточнить мастер-флаг перед согласованием.
- Заголовки стендов в основной таблице Детализации и в AI-сводке возвращены к
  правому выравниванию, как у числовых значений.
- Risk-бары получили больший безопасный gap до суммы и более ранний
  container-query перенос суммы/процента под бар на узкой карточке.
- Добавлены regression guards: unit для `ai_llm_used=false + explicit token
  answers`, e2e для Dashboard/Details token visibility, e2e для правого
  выравнивания стендов и e2e для отсутствия bar/amount collision.

## 25.05.2026 · PATCH 2.20.65 — зелёный статус сохранения

**Контекст.** Индикатор `Все изменения сохранены` визуально оставался
нейтрально-серым, хотя по смыслу это успешное состояние. Пользователь
воспринимает его как кнопку/плашку статуса и ожидает зелёный success-state.

**Решение.**

- Для постоянного `persist-idle` и краткого `persist-saved` задан единый
  success-вид: зелёный текст, зелёная рамка и `success-faint` фон.
- Добавлен unit guard `persist-indicator-success-state.test.js`, который
  проверяет, что сохранённое состояние нельзя случайно вернуть в серый стиль.

## 25.05.2026 · PATCH 2.20.64 — токены в Детализации и выравнивание UI

**Контекст.** Пользовательский JSON и legacy-расчёты могли иметь заполненный
раздел LLM-токенов, но строки «Входящие/исходящие токены LLM» в Детализации
оставались `—`/`0 ₽`, потому что стоимость считалась только по сохранённым
`qtyFormulas`. UI-fallback 2.20.63 восстанавливал summary/qty, но не менял
единый расчётный результат. Дополнительно в Детализации заголовки стендов были
визуально смещены, а в карточке «Вклад риск-коэффициентов» бары имели разные
левые края и длину.

**Решение.**

- В `calculate()` добавлен доменный fallback для внешних LLM-token ЭК:
  если формула вернула 0, но ответы пользователя описывают положительную
  внешнюю/private-cloud LLM-нагрузку, qty и стоимость восстанавливаются из
  тех же ответов, что seed-формулы. Для `on_prem_gpu` внешний token-биллинг
  остаётся 0.
- «Детализация → Сводка AI-метрик» теперь следует выбранной подвкладке:
  в режиме `Бюджет (₽)` показывает ₽/мес по AI-метрикам, в режиме `Объём (qty)`
  показывает токены/ГБ/vCPU.
- e2e `desktop-details-ai-tokens.spec.js` теперь проверяет не только
  «не прочерк», но и положительные `₽` в строках токенов, положительные qty и
  непустую «Сводку AI-метрик» после обнуления token-формул.
- Заголовки стендов в таблицах Детализации и «Сводке AI-метрик» центрируются и
  проверяются e2e по фактическим координатам колонок.
- Для строк риска задана фиксированная колонка подписи и фиксированная колонка
  бара: все бары начинаются в одной x-координате и имеют одинаковую ширину.
- Для узких карточек добавлен container-query layout: бар остаётся выровненным,
  а сумма и процент уходят на строки ниже вместо обрезки справа.
- Добавлен e2e-guard `dashboard-risk-bars-layout.spec.js`: проверяет координаты
  `left` и `width` всех баров и отсутствие выхода label/amount/% за границы
  карточки.
- На пользовательском JSON `repaired-v2.20.63` контрольный расчёт даёт
  положительные токены и стоимость: input LLM 1/1/2/15/15 млн токенов по
  DEV/ИФТ/ПСИ/ПРОМ/Нагрузка, output LLM 1/1/1/4/4 млн токенов.

## 25.05.2026 · PATCH 2.20.63 — видимость qty в Dashboard/Details и fallback AI-нагрузки

**Контекст.** Пользователь проверял токены не только в отдельном блоке
«Объёмы AI-нагрузки», но и в рабочем блоке «Объёмы ресурсов» на Dashboard, а
также в «Детализация → Сводка AI-метрик». Тесты 2.20.62 подтверждали отдельный
AI-блок, но не защищали ожидание «Токены видны там, где пользователь ищет
ресурсные объёмы».

**Решение.**

- Строка **Токены** добавлена в Dashboard-блоки «Объёмы ресурсов» для ИТОГО и
  стендов, если рассчитана положительная LLM-нагрузка.
- AI-агрегатор получил fallback: если token-ЭК по legacy/повреждённой формуле
  дают 0, но ответы раздела LLM дают положительную нагрузку, `TOKENS`
  восстанавливается из ответов и становится видимым в Dashboard и Детализации.
- Fallback расширен с токенов на операционные AI-метрики, которые пользователь
  ожидает видеть как нагрузку: `TOKENS` и `EMBEDDINGS`. Для fallback применяется
  тот же capacity-множитель, что и в «Детализация → Объём (qty)».
- «Детализация → Объём (qty)» получила явный `data-item-id` на строках ЭК, чтобы
  e2e проверял не только модель, но и фактическую видимость конкретной строки.
- e2e-контракт теперь специально обнуляет token-формулы и проверяет, что
  «Детализация → Сводка AI-метрик» всё равно показывает токены.
- Добавлен e2e-контракт Quick Start: все положительные по модели resource/AI
  строки должны быть видимыми и непустыми в Dashboard, а все ЭК с положительным
  qty должны иметь непустую строку в «Детализация → Объём (qty)». Отдельно
  покрыт Quick Start с собственной GPU-моделью, чтобы не потерять GPU,
  токены и эмбеддинги.
- Дополнительно вручную проверен пользовательский JSON в браузерном DOM:
  `Токены = 45 млн токенов / мес` в сводке Детализации и resource-блоке
  Dashboard, входящие/исходящие токены и эмбеддинги имеют непустые строки в
  «Детализация → Объём (qty)».

## 25.05.2026 · PATCH 2.20.62 — видимость AI-токенов и SSD/HDD по всем стендам

**Контекст.** Пользовательский импортированный JSON давал ненулевые AI-токены и
хранилища, но Dashboard мог скрывать малые SSD/HDD как `—`: дробные значения в
ТБ округлялись как целые ресурсы, а HDD был рассчитан только для ПСИ/ПРОМ.
Из-за этого экран выглядел так, будто DEV, ИФТ и Нагрузка остались без
хранилища, хотя модель уже считала часть объёмов.

**Решение.**

- HDD рассчитывается для всех активных стендов: DEV, ИФТ, ПСИ, ПРОМ, Нагрузка.
  Для непроизводственных стендов используется резервный объём БД с минимальным
  порогом, для ПРОМ сохранён прежний учёт холодной доли файлов.
- Dashboard больше не округляет ТБ как штуки: дробные объёмы `0.x ТБ`
  остаются видимыми в блоках «Объёмы ресурсов» и карточках стендов.
- e2e-контракт на Quick Start/импортированный JSON теперь явно проверяет
  видимость токенов в Dashboard и Details, а также SSD/HDD для DEV, ИФТ и
  Нагрузка.
- Детализация → Объём (qty) переведена на ту же семантику количества, что и
  Dashboard: при включённых рисках qty включает capacity-буферы
  задачи/проекта/сезонности/сдвига/контингента, без VAT и инфляции.
- Добавлен e2e-инвариант сходимости Dashboard ↔ Детализация по общим суммам,
  суммам стендов, категориям, количеству ненулевых ЭК в категориях и
  qty-агрегатам.

**Проверки.** Целевые UI/domain/e2e проверки, `test:ui`, `test:quick`,
`test:integration`, `test:architecture`, `sanity:check`,
`quantity:audit:check`, `syntax-check`. Полный релизный прогон фиксируется
отдельно перед выпуском.
...
на Дашборде в блоке «Объёмы AI-нагрузки» значение `Токены` могло быть нулевым
при `ai_hosting_mode = on_prem_gpu`. Причина: Дашборд агрегировал `TOKENS`
из стоимостных ЭК `llm-tokens-input-1m` / `llm-tokens-output-1m`. Эти ЭК
намеренно обнуляются для собственной GPU-инфраструктуры, чтобы не начислять
плату за внешний LLM API, но вместе с бюджетной строкой терялась операционная
метрика нагрузки.

**Решение.**

- `aggregateAiMetrics` получил контекст расчёта и достраивает операционные
  `TOKENS` / `EMBEDDINGS` из ответов Опросника для on-prem LLM/RAG, если
  стоимостные token/embedding ЭК равны нулю.
- Внешние token-cost строки при `on_prem_gpu` остаются нулевыми: исправление
  не добавляет плату за внешний API, а только показывает объём AI-нагрузки.
- Дашборд, Детализация и Сравнение передают `calc` в общий агрегатор, поэтому
  AI-сводки читают одну и ту же модель.
- UserManual уточняет, что блок «Объёмы AI-нагрузки» — операционный, а не
  только бюджетный: при собственной GPU-инфраструктуре `Токены` видны как
  нагрузка без начисления external API cost.

**Проверки.**

- Unit: `dashboard-aggregates` проверяет on-prem LLM: external token item qty
  остаётся `0`, а Дашборд показывает `2040 млн токенов / мес`.
- E2E: Quick Start AI показывает `Токены` на Дашборде и в Детализации; on-prem
  GPU сценарий показывает `Токены` на Дашборде, `Бюджет (₽)` и `Объём (qty)`.

## 25.05.2026 · PATCH 2.20.57 — Details token visibility contract

**Контекст.** В пользовательском сценарии раздел «Объём токенов» был заполнен,
ядро считало `llm-tokens-input-1m` / `llm-tokens-output-1m`, но первый экран
`Детализация → Бюджет (₽)` не показывал агрегат «Токены»: AI-сводка была
загейтена только на подвкладку `Объём (qty)`, а item-строки находились внутри
свёрнутой категории `AI / LLM`. Для пользователя это выглядело как «токены не
рассчитаны».

**Решение.**

- `Сводка AI-метрик` выводится на обеих подвкладках Детализации: `Бюджет (₽)`
  и `Объём (qty)`.
- Кнопка `Скрыть без бюджета` продолжает синхронно скрывать пустые AI-строки
  (`CPU агентов` без агентского режима), но не скрывает рассчитанные `Токены`.
- Calculation Health получил ошибку `ai-token-volume-without-llm`: если поля
  объёма токенов явно изменены, но `ai_llm_used` выключен, расчёт больше не
  молчит о причине нулевых token-строк.
- Quick Start contract усилен: все AI-комбинации должны давать ненулевые LLM
  token qty на каждом стенде, все non-AI-комбинации — нулевые.
- UserManual обновлён: AI-сводка в Детализации описана как видимая на обеих
  подвкладках.

**Проверки.**

- focused domain/ui/e2e: `calculation-health`, `wizard-quickstart-calculation`,
  `details-ai-summary-subtab-gate`, `desktop-details-ai-tokens`.
- браузерная проверка на пользовательском repaired JSON: на `Бюджет (₽)` видна
  AI-сводка с `Токены`, на `Объём (qty)` те же значения сохраняются, после
  `Скрыть без бюджета` пустой `CPU агентов` скрыт, `Токены` остаются.
...
недостаточны: в `Сводка AI-метрик` строки с одними `—` не скрывались кнопкой
`Скрыть без бюджета`, а в PDF Опросника секция `Параметры расчёта` съезжала
по колонкам, особенно в extended-режиме с колонкой `Пояснение`.

**Решение.**

- `Сводка AI-метрик` теперь принимает `hideNoBudget` и скрывает строки по
  фактически видимым значениям на активных стендах и в `ИТОГО`, а не по
  косвенной стоимости исходной ЭК.
- Print-стили AI-сводки сбрасывают фон/цвета для dark-theme PDF и убирают
  экранный `flex` внутри table-cell, чтобы строки `tbody` были видны в PDF.
- PDF Опросника больше не строит `Параметры расчёта` отдельной таблицей:
  секция стала `tbody` той же таблицы `Вопрос / Ответ / Пояснение`, поэтому
  Chromium PDF не пересчитывает ширины колонок отдельно.
- E2E `desktop-export-print.spec.js` проверяет оба пользовательских режима:
  AI-сводку в dark print и выравнивание `Параметры расчёта` координатами в
  compact и extended PDF.
- В `AGENTS.md` зафиксирован QA-урок: PDF-баг закрывается только после
  реального PDF→PNG/raster просмотра того же пользовательского режима.

**Проверки.**

- `npm run syntax-check`
- focused unit/print guards: 56 passed
- `desktop-export-print.spec.js`: 8 passed
- real PDF→PNG: `questionnaire-extended-actual-*.png`,
  `ai-summary-actual-*.png`

## 23.05.2026 · PATCH 2.20.53 — Factor icon + Details PDF theme print fix

**Контекст.** Для кнопки `Анализ факторов` нужен более точный визуальный знак:
`bar-chart-3` читался как обычный график/отчёт, а не как анализ влияющих
параметров. После этого пользователь показал дефект PDF-печати Детализации:
справа от таблицы оставалась цветная полоса темы.

**Решение.**

- В локальный набор иконок добавлена Lucide-иконка `git-branch`.
- Кнопка `Анализ факторов` использует `git-branch`, чтобы визуально показывать
  ветвление фактора в зависимые элементы расчёта.
- Добавлена source-защита, чтобы кнопка не вернулась к абстрактной диаграмме.
- Для печати отключены transitions/animations: PDF не должен успевать схватить
  промежуточный цвет светлой или тёмной темы.
- `.details-table-wrap` / `.items-table-wrap` в print принудительно получают
  белый фон, нулевой radius и без тени; служебная колонка `.col-info`
  скрывается на печати.
- E2E-проверка Details PDF теперь явно прогоняет и `dark`, и `light` темы.
...
Прогресс (ветка `qty-model-prom-rag-llm`, без bump/release до общего ревью):
...
×1.3, WAL +10%, сжатие бэкапов ÷2, доли горячих/холодных файлов, версии S3). При
импорте старого расчёта показывается health-info `storage-model-assumptions-updated`
(условие 2), чтобы сдвиг не выглядел как тихая магия.

## Release 2.21.0 — доработка qty-модели ПРОМ (RAG / LLM / Storage / CPU-RAM) (2026-06-17)

### Контекст

Накопленный релиз Этапов 1-4 доработки qty-модели ПРОМ (см. выше «ПЛАН» + review-проход).
Stage 5 (Security/DR/project) в этот релиз НЕ входит — отдельным пакетом после.

### Что изменилось

- **RAG:** размер vector-DB больше не зависит от числа поисков; число эмбеддингов
  считается из корпуса (с ручным override); realtime = непрерывная дельта (×2);
  новый параметр доли корпуса за цикл; retrieval → эмбеддинги запросов.
- **LLM:** опциональный детальный режим входных токенов (системный промпт / запрос /
  история / RAG-контекст / контекст инструментов); `ai_safety_overhead_percent` (0-50%)
  вместо хардкода 10%; уточнено `ai_caching_share`.
- **Storage:** SSD учитывает индексы БД (×1.3) и WAL (+10%); HDD-бэкап делится на
  компрессию (÷2); объектное хранилище — версии; параметризованы доли горячих/холодных
  файлов и объём БД на пользователя.
- **CPU/RAM:** единая база vCPU (одно определение для CPU и RAM); опциональные
  расширенные модели CPU (от CPU-времени запроса) и RAM (app baseline + realtime); ПОРОГ
  min_instances; realtime теперь `max(1, ceil(PCU/1000))`.
- **+8 новых health-проверок** (RAG/LLM/Storage/CPU-RAM согласованность).
- **Объяснимость:** все новые ответы и производные видны в трассе Паспорта ПРОМ.

### ⚠ Намеренные сдвиги сумм (НЕ баги — новая модель честнее)

- **Storage:** суммы по **SSD / HDD / объектному хранилищу** могут измениться (индексы
  БД, WAL, компрессия бэкапов, доли файлов, версии). При импорте старого расчёта —
  health-info `storage-model-assumptions-updated`.
- **realtime CPU/RAM:** при **PCU ≥ 1000** число ядер под realtime растёт пропорционально
  (`ceil(PCU/1000)` вместо прежнего фиксированного +1) — CPU/RAM крупных realtime-расчётов
  могли вырасти. При PCU < 1000 без изменений.
- Все новые расширенные параметры/режимы **опциональны и выключены по умолчанию** — для
  расчётов без RAG/AI и без расширенных режимов модель почти не меняется.

### Проверки (финальные gates после bump + merge)

- `npm test`: см. финальный прогон в release-commit.
- `npm run smoke:desktop` (AI/RAG/Dashboard/Details): 11/11.
- `quantity:audit:check` / `sanity:check` / `prices:freshness:check` / `syntax-check`
  / `git diff --check`: pass.

### Версионирование

`2.20.119 → 2.21.0` (**MINOR**): расчётная qty-модель изменена, есть намеренные сдвиги
сумм (storage, realtime CPU/RAM при PCU≥1000). Schema/provider-формат не меняются.
5 коммитов: `b47c1f9` (RAG), `cbbfe92` (LLM), `351fc64` (Storage), `b025180` (CPU/RAM),
`1377507` (review-fix).
