...
- [seed.js:SEED_QUESTIONS](js/domain/seed.js#L48) — текущий каталог 124 вопроса

---

## 1. Цель

Снизить барьер входа: вместо 124 полей пользователь отвечает на **7 макро-вопросов** и получает полностью предзаполненный опросник + рабочую смету. Архитектор остаётся primary пользователем — все 124 поля видны в Опроснике, помечены бейджем «Из профиля» и доступны для override.
