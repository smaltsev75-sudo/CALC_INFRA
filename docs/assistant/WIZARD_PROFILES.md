...
- [seed.js:SEED_QUESTIONS](js/domain/seed.js#L53) — текущий каталог 130 вопросов

---

## 1. Цель

Снизить барьер входа: вместо 130 полей пользователь отвечает на **7 макро-вопросов** и получает полностью предзаполненный опросник + рабочую смету. Архитектор остаётся primary пользователем — все 130 полей видны в Опроснике, помечены бейджем «Из профиля» и доступны для override.
...
**НЕ заполняется** (69 для B2B-standard):
