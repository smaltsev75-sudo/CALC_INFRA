...
- [seed.js:SEED_QUESTIONS](js/domain/seed.js#L48) — текущий каталог 90 вопросов

---

## 1. Цель

Снизить барьер входа: вместо 90 полей пользователь отвечает на **7 макро-вопросов** и получает полностью предзаполненный опросник + рабочую смету. Архитектор остаётся primary пользователем — все 90 полей видны в Опроснике, помечены бейджем «Из профиля» и доступны для override.
