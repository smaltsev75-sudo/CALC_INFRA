...
- [seed.js:SEED_QUESTIONS](js/domain/seed.js#L53) — текущий каталог 132 вопросов

---

## 1. Цель

Снизить барьер входа: вместо 132 полей пользователь отвечает на **7 макро-вопросов** и получает полностью предзаполненный опросник + рабочую смету. Архитектор остаётся primary пользователем — все 132 поля видны в Опроснике, помечены бейджем «Из профиля» и доступны для override.
...
| 7 | `wz_ai_used` | boolean | `true` / `false` | `false` | Включает весь AI-блок (LLM/RAG/агенты), иначе все 37 вопросов AI = «не используется» |
...
**НЕ заполняется** (72 для B2B-standard):
- Все `budget` (target_capex, target_opex, launch_year, schedule_shift, deployment_cost_override_mrub, staff_training_cycles) — это плановые/проектные оценки, остаются default
- AI-агенты (`ai_agent_mode`, `agent_*`) — слишком специфично, оставляем `false`/default
- `ai_finetune_*` — оставляем `false`
- **Параметры объёма журналов аудита** (`audit_events_per_day`, `audit_bytes_per_event`, `audit_retention_years`, `audit_log_compression_ratio`, добавлены в 5B-Sec audit-log completeness) — wizard их НЕ заполняет: остаются на default (`events=0` → грубая fallback-оценка «доля от БД»). Точную event-модель пользователь задаёт вручную в Опроснике при необходимости.
- **Параметры масштаба SIEM** (`siem_log_gb_per_day`, `siem_sources_count`, `siem_tier`, добавлены в 5B-Sec SIEM scaling) — wizard их НЕ заполняет: остаются на default (`log=0`, `sources=0`, `tier=basic` → flat-оценка «один базовый контур + один проект»). Масштаб по объёму логов / числу источников пользователь задаёт вручную при необходимости.
- **Класс защиты от DDoS** (`ddos_tier`, добавлен в 5B-Sec DDoS tier-select) — wizard его НЕ заполняет (auto-tier намеренно не делаем, чтобы golden не дрейфил): остаётся на default `basic_l3_l4` (×1 = базовая L3/L4-стоимость). L7/premium пользователь выбирает вручную; для критичных профилей показывается health-info.
- **Число защищаемых доменов WAF** (`waf_domains_count`, добавлен в 5B-Sec WAF domains scaling) — wizard его НЕ заполняет: остаётся на default `0` (→ оценка на 1 домен, baseline). Multi-domain пользователь указывает вручную; при default показывается health-info.
- **Число рабочих мест и каналов DLP** (`dlp_protected_users_count`, `dlp_channels_count`, добавлены в 5B-Sec DLP seats/channels) — wizard их НЕ заполняет: остаются на default `0` (→ один лицензионный контур + один проект внедрения, baseline). Масштаб по числу рабочих мест (≈500 на контур) и каналов контроля (≈3 на блок) пользователь задаёт вручную; при default показывается health-info.
- **Прямой объём защищаемых данных** (`protected_data_volume_gb`, `protected_data_growth_gb_year`) — wizard их НЕ заполняет: остаются на default `0` (→ прежняя грубая оценка от БД/пользователей). Пользователь задаёт их вручную, когда защищаемое хранилище относится только к отдельным данным, например сканам документов.

**Решение по AI при `wz_ai_used = false` (зафиксировано):**

Если в Quick Start пользователь выбрал «AI не используется» — **никакие AI-defaults не подтягиваются вообще**. Все 37 AI-вопросов остаются без значений (фактически `null` / unset). Если пользователь позже вручную включит `ai_llm_used=true` в детальном Опроснике, **AI-секция стартует пустой**, и пользователь сам заполняет параметры. Это честно (никаких скрытых дефолтов из «спящего» wizard'а) и предотвращает путаницу — если бы дефолты были заранее заполнены, пользователь мог бы получить смету с AI-расходами, не понимая, откуда они взялись.
...
5. **Валидация AI-блока:** если `wz_ai_used=false`, все 37 AI-вопросов получают значения «выключено / 0». Если позже пользователь включит `ai_llm_used` вручную — нужны ли разумные defaults? **Предложение:** подтянуть AI-defaults даже при `wz_ai_used=false` (чтобы при включении в Опроснике сразу был рабочий AI-сценарий с industrial-tuning).
...
### 13.6 Provider price overlay — текущее состояние

**Состояние (v2.22.31, net-цены после Stage VAT-2).** Overlay-цены живут в
[`js/data/providers-bundled.generated.js`](../../js/data/providers-bundled.generated.js) (генерируется из
`data/providers/*-latest.json` через `npm run generate:providers`) — это **единственный источник истины**.
Полный per-SKU прайс здесь намеренно не дублируется, чтобы документация не расходилась с runtime
(прежняя таблица «14 ЭК / cloud.ru/services 2026-Q2» устарела — удалена).

Текущие overlay'и по числу SKU: Cloud.ru/`sbercloud` — **16 SKU**, Yandex/`yandex` — **15 SKU**, VK/`vk` — **10 SKU**.

| Провайдер (`providerId`) | SKU | `license-db-per-vcpu` (DB-лицензия) |
|---|---|---|
| Cloud.ru (`sbercloud`) | 16 | нет в overlay → seed-fallback (Tantor SE 167 000 ₽/vCPU/год) |
| Yandex (`yandex`) | 15 | нет в overlay → seed-fallback (Tantor SE) |
| VK (`vk`) | 10 | **есть:** MS SQL Enterprise — 598 214.75 ₽/vCPU/год net (gross 729 822) |

**Единицы лицензий — годовые, не месячные:** `license-db-per-vcpu` = ₽/vCPU/год; `license-os-per-node` и
`license-siem-edr-per-node` = ₽/узел/год. (Compute/Storage/Network/AI — ₽/мес, ₽/ГБ·мес, ₽/ТБ·мес.)

ЭК, отсутствующий в overlay выбранного провайдера, берёт цену из seed (fallback). Поэтому DB-лицензия
у Cloud.ru/Yandex считается по seed-baseline Tantor SE; цену Postgres Pro Enterprise / Oracle /
MS SQL Enterprise задавайте через VK-overlay, импорт прайса провайдера или КП.

**Подключение:** overlay применяется в `js/domain/providerOverlay.js` (`buildOverlayPricesFromBundled(providerId)`),
цены поступают в `calculator.js: calculate()` до основного цикла. Все consumer'ы (Дашборд / Детализация /
Сравнение / PDF / CSV) получают provider-aware цены из единого источника.
