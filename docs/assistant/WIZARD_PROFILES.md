...
- [seed.js:SEED_QUESTIONS](js/domain/seed.js#L48) — текущий каталог 119 вопросов

---

## 1. Цель

Снизить барьер входа: вместо 119 полей пользователь отвечает на **7 макро-вопросов** и получает полностью предзаполненный опросник + рабочую смету. Архитектор остаётся primary пользователем — все 119 полей видны в Опроснике, помечены бейджем «Из профиля» и доступны для override.
