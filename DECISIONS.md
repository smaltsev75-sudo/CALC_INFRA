...
## 23.05.2026 · PATCH 2.20.52 — Root-cause factors wording polish

**Контекст.** Пользователь указал, что окно корневых причин говорит языком
разработчика: названия факторов обрезаются, а подписи `Проверка`, `Значение`,
`Больше всего меняются` и фраза про числовые параметры непонятны.

**Решение.**

- Кнопка в Детализации переименована в `Анализ факторов`.
- Окно переименовано в `Анализ факторов`; вводная строка теперь говорит:
  `Top-X корневых причин, влияющих на расчёт и бюджет`.
- Подписи строк переписаны на пользовательский язык: `Влияние на бюджет`,
  `Что меняем для оценки`, `Влияет на`, `Сейчас → для оценки`,
  `ЭК с самым большим изменением бюджета`.
- Название фактора больше не обрезается через ellipsis: длинный текст
  переносится на следующую строку.

## 23.05.2026 · PATCH 2.20.51 — Root-cause budget drivers modal

**Контекст.** После v2.20.50 пользователь уточнил, что нужен не top-10 самых
дорогих ЭК, а top-X корневых причин: параметры, от которых зависят другие
части расчёта и которые могут драматически оптимизировать итоговый бюджет.

**Решение.**

- В Детализации добавлена компактная кнопка `Причины бюджета`.
- Отдельное окно `Корневые причины бюджета` показывает не ЭК, а параметры
  расчёта, которые дают наибольшую экономию при проверочном изменении.
- Модель `rootCauseAnalysis` делает реальные пересчёты: числовые параметры
  проверяются снижением на 10%, булевы — переключением. В отчёт попадают только
  причины, где бюджет реально снижается.
- Для каждой причины показывается месячная/годовая экономия, доля от бюджета,
  сколько ЭК меняется и какие ЭК дают основной вклад изменения.
- Свёрнутый блок `Связи и изменения` показывает исходное/проверочное значение,
  прямые связи в qty-формулах и top изменившихся ЭК.
- Print/PDF путь очищен от этого аналитического блока. В PDF остаётся только
  отдельная проверка количества ЭК, если пользователь включил её в параметрах
  печати Детализации.
- Обновлены unit/source guards и desktop/PDF E2E-регрессии.

## 23.05.2026 · PATCH 2.20.50 — Details cost calculation check

**Контекст.** Пользователь попросил проверить логику расчёта ЭК при Quick
Start и обычном опроснике: почему берутся конкретные коэффициенты, как один
параметр влияет на другой, как из количества ЭК получается бюджет, и где
контролируются единицы измерения.

**Решение.**

- В Детализацию добавлен блок `Проверка расчёта ЭК` по top-10 самым дорогим
  ЭК текущего расчёта.
- Для каждой строки показывается вклад за месяц/год и цепочка по самому
  дорогому стенду: `qty × цена × период × риски × НДС = ₽/мес`.
- В строке выводятся источники ответов (`Quick Start`, опросник, значения по
  умолчанию) и ключевые коэффициенты стенда.
- Добавлена проверка единиц: `costBase` должен сходиться с
  `qty × pricePerUnit × billingIntervalMul`, а `costFinal` — с рисками и НДС.
- Добавлен документ [BUDGET_CALCULATION.md](BUDGET_CALCULATION.md) и расширен
  [QUANTITY_LOGIC_AUDIT.md](QUANTITY_LOGIC_AUDIT.md) блоком про единицы и
  месячный множитель тарифа.
- Добавлены unit и desktop/PDF E2E-регрессии.

## 23.05.2026 · PATCH 2.20.49 — Details PDF quantity check option

**Контекст.** Пользователь попросил дать выбор: печатать в PDF Детализации
блок `Почему столько? Проверка количества ЭК` или выгружать PDF без него.

**Решение.**

- Для PDF на вкладке `Детализация` добавлено окно `Параметры PDF детализации`.
- По умолчанию проверка количества ЭК включена, чтобы сохранить поведение
  v2.20.47/v2.20.48.
- Пользователь может снять чекбокс `Печатать проверку количества ЭК`; тогда
  print-mode добавляет класс `printing-details-no-quantity-summary`, а
  `print.css` скрывает только печатный блок qty-check.
- Добавлены unit и desktop/PDF E2E-регрессии на оба варианта.
...
почему используются конкретные коэффициенты, как ответы Quick Start и обычного
опросника влияют на qty, и как это связано с бюджетом.

**Решение.**

- В строках Детализации под названием ЭК добавлена кнопка `Почему столько?`.
- Окно формулы теперь начинается с понятной трассировки: ответы пользователя,
  источник ответа (`вручную`, `Quick Start`, `по умолчанию`), коэффициенты стенда,
  итог qty и связь с месячной стоимостью.
- Трассировка использует production `quantityTrace`/`buildContext`, поэтому
  показывает те же эффективные `S.standSizeRatio.*`, что и реальный расчёт.
- PDF Детализации получил печатный блок проверки количества по top-ЭК, чтобы
  в выгрузке были видны ключевые входы qty-расчёта.
- Добавлены unit и desktop/PDF smoke-регрессии на кнопку, модалку, Quick Start
  sources и печатный блок.

## 23.05.2026 · PATCH 2.20.46 — Price Benchmark empty-cell dedupe

**Контекст.** Пользователь показал, что в пустых ячейках Прайс-бенчмарка
статус дублировался: `—` + `Нет цены` или `по запросу` + `По запросу`.

**Решение.**

- Для ячеек без цены больше не выводится прочерк.
- Для цен `по запросу` больше не выводится отдельная текстовая строка над
  бейджем.
- Пустая ячейка показывает один статусный бейдж: `Нет цены` или `По запросу`.

## 23.05.2026 · PATCH 2.20.45 — Price Benchmark hint placement

**Контекст.** Пользователь уточнил, что строка про смысл чисел должна быть
под таблицей, а объяснение выбора ЭК не должно занимать отдельное место на
экране.

**Решение.**

- Легенда `Крупное число... / строка «цена»...` перенесена под основную таблицу
  Прайс-бенчмарка.
- Пояснение выбора ЭК теперь показано как hover-hint на подписи
  `ЭК в сравнении:`:
  `Показаны top-6 самых дорогих ЭК текущего расчёта, имеющие стоимость в
  прайсе Cloud.ru`.
- Видимый текст строки фильтра снова компактный: подпись + выбранные ЭК.

## 23.05.2026 · PATCH 2.20.44 — Price Benchmark one-table layout

**Контекст.** Пользователь отменил отдельные верхние таблицы в
Прайс-бенчмарке: они повторяли список провайдеров и съедали место на экране.
Нужна одна основная таблица, где сразу видны провайдер, дата прайса, статусы
цен, вклад ЭК и цена за единицу.

**Решение.**

- Верхние таблицы `Актуальность прайсов` и `Cloud.ru vs Yandex vs VK:
  доверие к ценам` больше не рендерятся в Прайс-бенчмарке.
- Дата актуальности прайса показана в строке провайдера основной таблицы:
  `Дата прайса: ДД.ММ.ГГГГ`.
- Строка `ЭК в сравнении` теперь объясняет выбор колонок: показаны до 6 самых
  дорогих ЭК текущего расчёта, только если по ним есть публичная цена Cloud.ru.
- Верхний hint оставлен только для расшифровки чисел в ячейках: крупное число
  — вклад за месяц, строка `цена` — тариф за единицу ресурса.

## 23.05.2026 · PATCH 2.20.43 — Price Benchmark clarity and actuality table

**Контекст.** Пользователь указал на три UI-проблемы Прайс-бенчмарка:
заголовки колонок визуально смещены относительно данных, пользователь не
понимает смысл чисел в строках, а актуальность прайсов смешана с основной
таблицей и должна быть вынесена выше матрицы доверия.

**Решение.**

- Актуальность прайсов вынесена в отдельную таблицу `Актуальность прайсов`
  выше блока `Cloud.ru vs Yandex vs VK: доверие к ценам`.
- В основной таблице удалено повторение даты в строке провайдера: теперь она
  показывает только провайдера, warnings, вклад ЭК и цену.
- Заголовки и значения денежных колонок выровнены по правому краю через
  full-width header content; desktop regression проверяет совпадение правых
  границ заголовка и ячейки.
- В каждой ячейке calc-specific режима видны две разные сущности:
  крупное число — `вклад, ₽/мес`, строка `цена:` — тариф за единицу ресурса.
- Hint переписан в терминах пользователя: «крупное число» и «строка цена»,
  без технических названий вроде `monthlyImpact`.

## 23.05.2026 · PATCH 2.20.42 — Price Benchmark visible sums

**Контекст.** Пользователь показал, что в Прайс-бенчмарке визуально «не
сходятся суммы»: заголовок и итог работали с месячным вкладом ЭК, но основное
число в ячейке показывало цену за единицу. Итоговая колонка складывала
`monthlyImpact`, а пользователь видел `pricePerUnit`, поэтому строка выглядела
как арифметическая ошибка.

**Решение.**

- В calc-specific режиме основное число ячейки теперь — месячный вклад ЭК у
  провайдера (`monthlyImpact`).
- Цена за единицу остаётся видимой, но вторичной строкой под основным числом.
- Заголовок колонки больше не показывает отдельное число вклада, чтобы не
  дублировать значение строки Cloud.ru и не смешивать уровни данных.
- Заголовки колонок и денежные значения выровнены вправо, чтобы названия,
  вклад и итог читались по одной вертикали.
- В DOM добавлены `data-monthly-impact` и `data-total-cost`; desktop regression
  проверяет, что сумма видимых вкладов строки Cloud.ru равна видимому итогу.

## 23.05.2026 · PATCH 2.20.41 — Cloud.ru baseline filter for Price Benchmark

**Контекст.** После перехода Прайс-бенчмарка на top-6 ЭК текущего расчёта
пользователь обнаружил логическую ошибку: в сравнение попадали дорогие позиции
вроде внедрения, пентеста или лицензий, если по ним нет публичной цены у
Cloud.ru/Yandex/VK. Таблица показывала в основном `Нет цены`, что не помогало
принять решение и создавало шум.

**Решение.**

- Cloud.ru выбран эталонным прайсом для выбора колонок Прайс-бенчмарка.
- `buildProviderBenchmarkItems()` теперь принимает `referencePrices` и оставляет
  только ЭК с известной положительной ценой Cloud.ru.
- Сортировка по-прежнему идёт по месячному вкладу ЭК в текущий расчёт, но только
  внутри множества позиций, которые можно сравнить с эталоном.
- Если calc-specific список пуст, `aggregateProviderPrices()` больше не
  подставляет fallback CPU/RAM/STORAGE/NETWORK/LICENSE: UI показывает честное
  сообщение, что нет ЭК с публичной ценой Cloud.ru для сравнения.
- В UI hint изменён на «до 6 крупнейших ЭК», потому что после фильтра по
  эталонной цене колонок может быть меньше шести.

**Тесты.**

- Domain-test защищает кейс: самый дорогой ЭК без эталонной цены Cloud.ru не
  попадает в Прайс-бенчмарк.
- Domain-test защищает пустой calc-specific список от отката к старым
  fallback-категориям.
- Desktop regression больше не требует ровно 6 колонок: проверяет диапазон
  `1..6` и отсутствие горизонтального overflow.

## 23.05.2026 · PATCH 2.20.40 — Calc-specific Price Benchmark + commercial license

**Контекст.** Пользователь указал, что Прайс-бенчмарк должен быть полезен для
конкретного расчёта, а не сравнивать абстрактные representative-позиции. В
частности, HDD отсутствовал, потому что старая модель показывала одну колонку
`STORAGE` через SSD-представителя, даже если в текущем расчёте холодное HDD
хранилище давало крупный вклад в бюджет. Дополнительно пользователь попросил
сменить тип лицензии: использование или модернизация продукта должны требовать
единоразового гонорара автору.

**Решение по Прайс-бенчмарку.**

- Старый fallback сохранён: без активного расчёта `aggregateProviderPrices()`
  возвращает прежние базовые категории CPU/RAM/STORAGE/NETWORK/LICENSE, чтобы не
  ломать старые вызовы и тесты.
- Для активного расчёта UI теперь строит `buildProviderBenchmarkItems()`:
  top-6 ЭК по `result.items[itemId].totalMonthly`, то есть по месячному вкладу
  на всём горизонте расчёта с учётом текущих формул, коэффициентов, НДС и
  интервалов тарификации.
- `HDD`, `RAM`, `CPU`, `SSD`, `WAF`, `SIEM/EDR` и другие привычные короткие
  термины используются в шапках колонок; полное название ЭК остаётся в tooltip.
- Для каждой колонки хранится `monthlyUsageFactor = monthlyCost / pricePerUnit`.
  Это позволяет пересчитать вклад ЭК на цену другого провайдера:
  `monthlyImpact = providerPricePerUnit × monthlyUsageFactor`.
- Колонка «Вклад ЭК» теперь является практической месячной оценкой видимых top-ЭК
  на текущих объёмах расчёта. Если по одному из видимых ЭК у провайдера нет
  публичной цены, итог помечается как `неполно`, чтобы UI не создавал ложную
  точность.
- Окно `modal-analytics` расширено до `min(1540px, calc(100vw - 32px))`, чтобы
  6 колонок top-ЭК умещались в desktop-сценарии без горизонтального скроллинга.

**Решение по лицензии.**

- `LICENSE` заменён с MIT на русскоязычную коммерческую пользовательскую
  лицензию.
- `package.json` и `package-lock.json` используют
  `"license": "SEE LICENSE IN LICENSE"`.
- README и UserManual больше не обещают свободное использование/модификацию:
  использование или модернизация требуют единоразового гонорара автору, размер
  гонорара определяется автором и согласуется сторонами.
- Явно записано, что выплата гонорара не включает обязательную кастомизацию,
  поддержку или выпуск новых версий; новый функционал и сопровождение могут быть
  заказаны отдельно.
- Добавлен отказ от финансовой и юридической ответственности автора за
  финансовые потери, упущенную выгоду и некорректные расчёты инфраструктуры или
  бюджета.

**Тесты и защита от регрессий.**

- Добавлены domain-проверки, что `buildProviderBenchmarkItems()` выбирает HDD,
  если HDD входит в крупнейшие ЭК текущего расчёта.
- Добавлена проверка, что `aggregateProviderPrices()` в calc-specific режиме
  считает `totalCost` через месячный вклад top-ЭК, а не через сумму unit-price.
- UI/source-тесты обновлены под расширенное окно и третий аргумент
  `benchmarkItems`.
- Bump: `2.20.39 → 2.20.40`. Schema, provider JSON schema и bundle format не
  менялись.
...
- в разделе «Быстрая проверка за 5-10 минут» все пункты отображались как `1.`;
- раздел «Горячие клавиши» был задублирован: один раз в `UserManual.md`, второй
  раз в отдельной секции, которую модалка справки добавляет из `HOTKEYS`.

**Причина.** Markdown-рендер справки поддерживает простые списки построчно.
Перенос строки внутри numbered-list item закрывал текущий `<ol>` и следующий
пункт начинал новый список, поэтому браузер снова показывал `1.`.

**Решение.**

- В [UserManual.md](UserManual.md) пункты «Быстрой проверки» сделаны
  однострочными, чтобы рендерились как один упорядоченный список из 6 пунктов.
- Оставшиеся многострочные bullet-пункты рядом также выровнены в одну строку,
  чтобы простой markdown-рендер не создавал рваную структуру.
- Секция горячих клавиш удалена из `UserManual.md`; единственный источник
  горячих клавиш в окне справки теперь [helpModal.js](js/ui/modals/helpModal.js),
  который строит таблицу из `HOTKEYS`.
- Unit-тест рендерит раздел «Быстрая проверка» через `renderMarkdown` и
  проверяет один `<ol>` и 6 `<li>`.
- Playwright regression проверяет, что в модалке ровно один заголовок
  «Горячие клавиши».

**Проверки.**

- Targeted Help/UserManual suite: 15/15 pass.
- Targeted Playwright F1 Help regression: pass.
- `npm test`: 5105/5105 pass.
- `npm run smoke:desktop`: 30/30 pass.
- `npm run pages:build`: pass.
- `npm run prices:freshness:check`: pass.
- `npm run sanity:check`: pass.
...
`2.20.37 → 2.20.38` (PATCH). Schema, provider JSON schema и bundle format не
меняются. Изменение пользовательски видимо как корректная нумерация списка
«Быстрая проверка» и отсутствие дубля «Горячих клавиш» в F1-справке.
...
`Уровень цен: Проверено` уже объяснял доверие к прайсу, но рядом с каждой
строкой цены повторялся бейдж `Проверено`. Пользователь справедливо указал,
что это снова дублирование информации в UI.

Дополнительно пользователь указал, что F1-справку тяжело читать: много
непрерывного текста, длинные абзацы и большой arrow-flow в code block вместо
сканируемого пользовательского маршрута.

**Решение.**

- В [providerPriceSummary.js](js/ui/providerPriceSummary.js) row-бейдж доверия
  теперь скрывается, если его статус совпадает с общим статусом прайса карточки.
- Row-бейджи остаются для исключений: `По запросу`, `Нет цены`, ручной ввод,
  оценка или любой статус, отличный от общего provider-level статуса.
- Playwright regression проверяет, что у Cloud.ru `Проверено` показано только
  в общем блоке, а не возле каждой цены.
- В [UserManual.md](UserManual.md) добавлен раздел «С чего начать», сокращён
  intro, а «Типовой сценарий использования» заменён с длинного моноширинного
  arrow-flow на обычный нумерованный список.
- В [css/modals.css](css/modals.css) F1-справка получила ограничение ширины
  строки, более плотную типографику, разделённые заголовки и читаемые таблицы.
- Устаревшие упоминания `Актуальность прайса: ДД.ММ.ГГГГ · версия ...` в
  справке заменены на date-only строку, чтобы документация не закрепляла
  прежнее дублирование даты и technical version.
- Hotkey label `F1` больше не говорит `Справка (README)`: in-app справка
  грузит `UserManual.md`.

**Урок усилен.** Общее правило «не дублировать информацию в UI» относится не
только к датам и tooltip'ам, но и к trust/status badges внутри одного scope.
Если общий блок уже сообщает статус, повторять тот же статус на каждой строке
нельзя; строка должна показывать только отличия от общего состояния.

**Проверки.**

- Targeted Help/UserManual/provider trust suite: 32/32 pass.
- Targeted provider price summary UI suite: 20/20 pass.
- Targeted Playwright provider price summary regression: pass.
- Targeted Playwright F1 Help readability regression: pass.
- `npm test`: 5101/5101 pass.
- `npm run smoke:desktop`: 30/30 pass.
- `npm run pages:build`: pass.
- `npm run prices:freshness:check`: pass.
- `npm run sanity:check`: pass.
...
`2.20.35 → 2.20.36` (PATCH). Schema, provider JSON schema и bundle format не
меняются. Изменение пользовательски видимо как отсутствие повторяющихся
`Проверено` возле каждой цены в карточке активного провайдера и более
читаемая F1-справка.
