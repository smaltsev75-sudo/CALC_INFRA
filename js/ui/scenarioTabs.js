...
function renderAddButton(ctx) {
    return el('button', {
        ...
        attrs: {
            type: 'button',
            /* Stage 4.16: дружелюбный tooltip объясняет ЗАЧЕМ создавать сценарий,
               а не ЧТО технически произойдёт (раньше было «откроется окно ввода
               имени» — описание механики, а не пользы). Текст отвечает на 3
               вопроса: альтернативные настройки / сравнить варианты / не потерять
               текущие данные. */
            title: 'Создайте новый сценарий, чтобы протестировать альтернативные настройки и сравнить разные варианты расчёта внутри одного калькулятора, не теряя текущие данные.'
        },
