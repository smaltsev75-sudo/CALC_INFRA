 ...
 *   - присутствует в state.ui.detailsCollapsedCats (массив явно записан), ИЛИ
 *   - массива нет (null) — дефолт «всё свёрнуто».
 * Поиск/фильтр (search, hideZero) НЕ переопределяют свёрнутое состояние —
 * пользователь сам решает, что разворачивать. Это сознательный выбор: иначе
 * любой поиск разворачивал бы все категории и накопленный «вид-настройка»
 * терялся бы.
 */
 ...
 */
function itemMonthlyOnActiveStands(itemId, result, disabledStands) {
...
/* Сводный блок AI-метрик внизу таблицы Детализации.
   ...
   Зачем здесь, а не только на Дэшборде:
     ...
     двух строк. */
function renderAiMetricsSummary(calc, result, disabledStands, applyRisks, ctx) {
    if (!calc) return null;
...
function renderQtySection(byCat, result, ctx, disabledStands = [], state = null, presentCats = []) {
...
function renderCostSection(byCat, result, ctx, totals, isFiltered, disabledStands = [], applyRisks = true, calc = null, state = null, presentCats = []) {
    const disabled = new Set(disabledStands);
    return el('div', { class: 'details-section' },
 ...
 */
function computeTotalsForItems(items, result, disabledStands = []) {
    const disabled = new Set(disabledStands);
    const stands = {};
