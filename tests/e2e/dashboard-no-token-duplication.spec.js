 ...
 *     CPU агентов.
 *   - В каждой карточке (Hero + 5 стенд-карточек) строка «Токены» встречается
 *     ровно 1 раз.
 */
...
test('Hero и стенд-карточки: «Токены» появляется ровно один раз на каждую карточку', async ({ page }) => {
    const consoleErrors = await bootCleanApp(page);
    await importHighAiCalc(page);
    await clickSidebarTab(page, 'dashboard');

    // Hero: считаем сколько раз строка с label «Токены» / «ТОКЕНЫ» встречается
    const heroCard = page.getByTestId('dashboard-hero');
    await expect(heroCard).toBeVisible();
    // Считаем по label-элементам (а не по тексту «Токены» где он может быть в tooltip)
    const heroTokenLabels = await heroCard
        .locator('.dash-resource-row-label, .dash-ai-metric-row-label')
        .filter({ hasText: /^токены$/i })
        .count();
    expect(heroTokenLabels,
        `Hero card: «Токены» должно быть ровно один раз (получено ${heroTokenLabels})`)
        .toBe(1);
...
test('«Токены» живёт ТОЛЬКО в блоке «Объёмы AI-нагрузки», не в «Объёмы ресурсов»', async ({ page }) => {
    ...
    const tokensInResources = await heroCard
    ...
    const tokensInAi = await heroCard
