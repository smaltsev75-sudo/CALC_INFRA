...
- [seed.js:SEED_QUESTIONS](js/domain/seed.js#L48) — текущий каталог 87 вопросов

---

## 1. Цель

Снизить барьер входа: вместо 87 полей пользователь отвечает на **7 макро-вопросов** и получает полностью предзаполненный опросник + рабочую смету. Архитектор остаётся primary пользователем — все 87 полей видны в Опроснике, помечены бейджем «Из профиля» и доступны для override.
...
**Правило:** если флаг compliance дополнительно требует ЭК, который сейчас отсутствует (DDoS / DLP / SSO / payment / antifraud / fine-tune / safety / TURN), wizard всё равно выставляет флаг — но **дополнительно показывает warning** «Этот компонент будет учтён в смете в следующих релизах». Это честно и не блокирует пользователя.
...
**НЕ заполняется** (29 для B2B-standard):
