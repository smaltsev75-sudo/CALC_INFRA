...
- **Опросник** из 130 вопросов про продукт (нагрузка, БД, AI, безопасность, регуляторика).
...
- **Анализ факторов** отдельным окном из Детализации: top-X корневых причин,
  влияющих на расчёт и бюджет, с пробным пересчётом и списком ЭК, где меняется
  стоимость. В PDF этот аналитический блок не печатается.
...
- **63 элемента конфигурации** (включая 13 AI-позиций) и формульный движок qty с собственным безопасным DSL.
- **Импорт прайса JSON** (provider-JSON, CSV или JSON-массив через мастер маппинга), история откатов, прайс-бенчмарк по top-6 ЭК текущего расчёта.
...
Проект распространяется по коммерческой пользовательской лицензии. Полный текст — в файле [LICENSE](LICENSE).

Использование продукта или его модернизация требуют единоразовой выплаты автору гонорара. Размер гонорара определяется автором и согласуется сторонами в ходе переговоров.

Оплата гонорара не включает обязательную кастомизацию, поддержку или выпуск новых версий. Новый функционал, доработка и сопровождение могут быть заказаны отдельно за дополнительную плату.

Продукт предоставляется "как есть": автор не несёт финансовой или юридической ответственности за финансовые потери, упущенную выгоду, некорректные расчёты инфраструктуры и/или бюджета.
...
что печатный режим через кнопку PDF, `Ctrl+Alt+P` и native `beforeprint`
включает A4 landscape и растягивает таблицы объёмов/стоимости на всю доступную
ширину листа.
...
«Задано вручную», «Оценка», «Частично», «Нет цены» или «По запросу». В
пользовательском UI рядом с прайсом показывается только дата актуальности без
технической версии provider JSON, а в прайс-бенчмарке есть до 6 ЭК текущего
расчёта по месячному вкладу среди позиций с публичной ценой Cloud.ru. Это
убирает из сравнения ЭК без эталонной цены; крупное число в ячейке бенчмарка
— вклад ЭК в расчёт за месяц, а строка «цена» ниже — тариф за единицу ресурса.
Хинт на строке `ЭК в сравнении` объясняет, что выбраны top-6 самых дорогих ЭК
текущего расчёта, имеющие стоимость в прайсе Cloud.ru. Дата прайса показана в
строке провайдера этой же таблицы, без отдельной верхней матрицы. На экранах с
бюджетом, стоимостью или сравнением
цен дата относится к прайсу, по которому выполнен конкретный расчёт
...
Все 63 элемента конфигурации в `js/domain/seed.js` имеют источник цены в inline-комментарии: 62 позиции с `pricePerUnit > 0` и 1 явно бесплатная позиция `traffic-ingress-tb` (входящий трафик, 0 ₽/ТБ):
